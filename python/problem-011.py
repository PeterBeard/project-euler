...
def max_products(grid, x, y):
    """Return the maximum directional product at point (x, y)"""
    up = 0
    down = 0
    left = 0
    right = 0
    ...
    return max([up, down, left, right, up_right, down_right])
...
def solution():
    ...
    for x in range(0, 20):
        for y in range(0, 20):
            max_product = max(max_product, max_products(grid, x, y))

    return max_product

print("The greatest product of adjacent numbers in any direction is {}".format(solution()))
